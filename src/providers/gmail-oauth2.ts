...
export class GmailOAuth2Provider extends EmailProvider {
  ...
  async initialize(): Promise<void> {
    ...
    this.oauth2Client = new google.auth.OAuth2(
      this.config.gmailOAuth2?.clientId,
      this.config.gmailOAuth2?.clientSecret,
      this.config.gmailOAuth2?.redirectUri || "urn:ietf:wg:oauth:2.0:oob"
    );
  ...
  validateConfig(): void {
    ...
    const emailUser =
      this.config.gmailOAuth2.emailUser || this.config.emailUser;
    if (!emailUser) {
      throw new Error(
  ...
  static async getRefreshToken(
    ...
    if (!tokens.refresh_token) {
      throw new Error(
        "No refresh token received. Make sure to include 'access_type: offline' and 'prompt: consent' in your auth URL."
      );
    }
